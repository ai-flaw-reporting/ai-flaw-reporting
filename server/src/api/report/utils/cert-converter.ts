...
function getSystemsArray(
  reporterDetails: ReshapedReportData["reporterDetails"],
): string[] {
...
function buildVulDescription(data: ReshapedReportData): string {
  const parts: string[] = [];

  const classify = data.classifyReport;
  if (classify) {
    const types: string[] = [];
    if (classify.real_world_harm) types.push("Incident");
...
function buildVulExploit(data: ReshapedReportData): string {
  const parts: string[] = [];
...
function buildVulImpact(data: ReshapedReportData): string {
...
function buildVulDiscovery(data: ReshapedReportData): string {
  const parts: string[] = [];
...
function buildDisclosurePlans(data: ReshapedReportData): string {
...
function determineIcsImpact(data: ReshapedReportData): boolean {
...
export function convertToCertObject(data: ReshapedReportData) {
...
export function convertToCertJson(data: ReshapedReportData): string {
  return JSON.stringify(convertToCertObject(data), null, 2);
}
