...
function processReport(data: ReshapedReportData): ProcessedReport {
  ...
  if (classify.real_world_harm) reportTypes.push("Incident");
  ...
  const security = data.securityDetails;
  ...
  const policyViolation = data.incidentDescription?.policyViolation;
  const policyViolationText = policyViolation?.reason ?? "Not specified";
  const policyViolationUrls = policyViolation?.urls ?? [];
  ...
  return {
    ...
    createdAt,
    flawDescription:
      data.incidentDescription?.issueDescription ?? "No description provided",
    ...
    reportTypes,
    reporterEmail: data.reporterDetails?.reporter?.email,
    securityData,
    evidenceData,
    ...
    disclosureChannels: data.reviewReport?.selectedStakeholders ?? [],
...
function serializeToJsonLd(report: ProcessedReport): Record<string, unknown> {
  ...
  const jsonld: Record<string, unknown> = {
    ...
    "flare:policyViolationUrls": report.policyViolationUrls,
  };
...
export function convertToJsonLdObject(
  data: ReshapedReportData,
): Record<string, unknown> {
  const processed = processReport(data);
  return serializeToJsonLd(processed);
}

export function convertToJsonLdJson(data: ReshapedReportData): string {
  return JSON.stringify(convertToJsonLdObject(data), null, 2);
}
