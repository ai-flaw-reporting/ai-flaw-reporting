...
export default function FormLayout({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    // Migrate old policyViolation.url (string) to urls (array)
    const pv = (
      savedFields as Record<string, Record<string, Record<string, unknown>>>
    ).incidentDescription?.policyViolation;
    if (
      pv &&
      typeof pv.url === "string" &&
      pv.url &&
      !("urls" in pv && pv.urls)
    ) {
      pv.urls = [pv.url];
